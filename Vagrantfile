...
hostfile_script = [].concat(masters, workers, nfs).collect { |data| "echo " + prefix + data[1].to_s + " " + data[0] + ".k8s.local " + data[0] + " >> /etc/hosts" }.join("\n")
...
Vagrant.configure("2") do |config|
  ...
  masters.each do |data|
    config.vm.define name="#{data[0]}.k8s.local" do |h|
      h.vm.hostname = name
